...
struct FoundationModelProviderTests {
    ...
    private let sut: FoundationModelProvider

    init() {
        mockFactory = MockFMSessionFactory()
        mockSession = MockFMSession()
        mockFactory.session = mockSession
        sut = FoundationModelProvider(sessionFactory: mockFactory)
    }
    ...
    @Test("send propagates errors from session")
    func send_sessionThrows_propagatesError() async throws {
        // Given
    ...
    @Test("identifier is foundation-models")
