...
| `benchmark` | `swift run Runner benchmark <provider\|all> [--runs N]` | Measures non-streaming latency, streaming time-to-first-token (TTFT), and streaming throughput (tokens/sec). Default: 10 measured runs per scenario (plus 3 discarded warm-up runs). |
