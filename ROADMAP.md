...
Eliminates the duplicated `URLSessionHTTPClient` and `HTTPClient` copies that currently ship independently inside `ClaudeProvider` and `OpenAIProvider`. Moving these into `AIProviderKitNetworking` means third-party provider authors get a production-ready HTTP + SSE client for free, without reinventing the wheel.

- [x] Promote `HTTPClient`, `HTTPRequest`, `HTTPResponse`, and `HTTPStreamError` into a standalone `AIProviderKitNetworking` library product
- [x] Promote `URLSessionHTTPClient` to `AIProviderKitNetworking` as the canonical `HTTPClient` implementation (Darwin-only via `#if canImport(Darwin)` for future Linux/Windows portability)
- [x] Remove the duplicate `HTTPClient` + `URLSessionHTTPClient` copies from `ClaudeProvider` and `OpenAIProvider`
- [x] Update both providers to import and use the shared implementation; expose `public init(authorization:httpClient:logger:)` for custom injection
